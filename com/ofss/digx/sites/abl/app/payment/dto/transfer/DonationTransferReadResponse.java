...
public class DonationTransferReadResponse extends BaseResponseObject implements IIndirection
{
  ...
  public DonationTransferReadResponse() {}
  ...
  {
    return "PayanyoneTransferReadRequestDTO [paymentId=" + paymentId + "transfer Details=" + transferDetails + "payee Details=" + payeeDetails + " ]";
  }
  ...
  public void indirectResponse(String sessionId)
  {
    ...
    try
    {
      ...
      {
        transferDetails.getDebitAccountId().setValue(masker.mask(transferDetails.getDebitAccountId().getValue(), "AccountNumberMasking", "account_id"));
        transferDetails.getDebitAccountId().setValue(indirectionHandler.create(sessionId, transferDetails.getDebitAccountId().getValue(), IndirectionType.ACCOUNT_ID));
      }
      if ((transferDetails != null) && (transferDetails.getPartyId() != null))
      {
        transferDetails.getPartyId().setValue(masker.mask(this.transferDetails.getPartyId().getValue(), "PartyIdMasking", "party_id"));
        transferDetails.getPartyId().setValue(indirectionHandler.create(sessionId, transferDetails.getPartyId().getValue(), IndirectionType.PARTY_ID));
      }
    }
    catch (Exception e)
    {
      logger.log(Level.SEVERE, formatter.formatMessage("Exception occurred in converting values of response DTO to thier respective Inditected-masked values.", new Object[0]), e);
    }
  }
